...
function Compare-Key($KeyName)
{
	...
	& $env:windir\Microsoft.NET\Framework\v4.0.30319\aspnet_regiis.exe -px $KeyName $myConfigkey -pri 
...
{ 
	...
	"Enable" {
		...
		{ 
			...
			& $env:windir\Microsoft.NET\Framework\v4.0.30319\aspnet_regiis.exe -pi "iisWasKey" $wasKey -exp
		}
		...
		{ 
			...
			& $env:windir\Microsoft.NET\Framework\v4.0.30319\aspnet_regiis.exe -pi $iisConfigKey $configKey -exp
		}
	...
	"Export" {
		...
		### Export Keys
		$wasKey = Join-Path $UNCPath $iisWASKey
		& $env:windir\Microsoft.NET\Framework\v4.0.30319\aspnet_regiis.exe -px $iisWASKey $wasKey -pri 

		$ConfigKey = Join-Path $UNCPath $iisConfigKey 
		& $env:windir\Microsoft.NET\Framework\v4.0.30319\aspnet_regiis.exe -px $iisConfigKey $ConfigKey -pri 
